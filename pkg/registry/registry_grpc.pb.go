...
type RegistryClient interface {
	...
	// 客户端监听服务实例变化
	Watch(ctx context.Context, in *WatchRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ServiceInstance], error)
	// 客户端发送心跳以续约租约
...
func (c *registryClient) Watch(ctx context.Context, in *WatchRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ServiceInstance], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Registry_ServiceDesc.Streams[0], Registry_Watch_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[WatchRequest, ServiceInstance]{ClientStream: stream}
...
type Registry_WatchClient = grpc.ServerStreamingClient[ServiceInstance]
...
type RegistryServer interface {
	...
	// 客户端监听服务实例变化
	Watch(*WatchRequest, grpc.ServerStreamingServer[ServiceInstance]) error
	// 客户端发送心跳以续约租约
...
func (UnimplementedRegistryServer) Watch(*WatchRequest, grpc.ServerStreamingServer[ServiceInstance]) error {
...
func _Registry_Watch_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(RegistryServer).Watch(m, &grpc.GenericServerStream[WatchRequest, ServiceInstance]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Registry_WatchServer = grpc.ServerStreamingServer[ServiceInstance]
